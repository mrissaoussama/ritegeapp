...
namespace ExpandableListView
{
    public partial class CollapsibleControl : ContentView
    {
        ...
        protected override void OnPropertyChanged([CallerMemberName] string propertyName = null)
        {
            ...
            else if (propertyName == Subtitle2Property.PropertyName)
            {
                if (string.IsNullOrEmpty(Subtitle2))
                {
                    Subtitle2Label.IsVisible = false;
                }
                else
                {
                    Subtitle2Label.TextColor = SubtitleTextColor;
                    Subtitle2Label.Text = Subtitle;
        ...
        #region Subtitle2 property       
        public static readonly BindableProperty Subtitle2Property =
                   BindableProperty.Create("Subtitle", typeof(string), typeof(CollapsibleControl), default(string), propertyChanged: OnSubtitleChanged);

        private static void OnSubtitle2Changed(BindableObject bindable, object oldValue, object newValue)
        {
            // Subtitle Property changed implementation goes here
        }
