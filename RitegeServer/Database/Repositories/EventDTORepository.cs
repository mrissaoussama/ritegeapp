...
namespace RitegeDomain.Database.Repositories
{
    public class EventDTORepository : GenericRepository<EventDTO>, IEventDTORepository
    {
        ...
        public async Task<List<EventDTO>> GetAlertsByIdSocieteAndDateAsync(int idSociete, DateTime dateStart, DateTime dateEnd)
        {
            ...
            {
                string query;
                query = "SELECT [indexEvent]      ,[dateEvent]      ,[HeureEvent]      ,[DoorNumber]      ,[userNumber]      ,[codeEvent]      ,[Flux]   FROM [controleaccessdb].[controleaccessdb].[event] e where e.doornumber in(select iddoor from [controleaccessdb].[controleaccessdb].door where idparking in (select idparking from parkingdb.dbo.parking where idparking=@idSociete)) and (dateEvent between @dateStart and @dateEnd) and codeevent " + EventCodes.GetAlertSqlString(); 
                System.Diagnostics.Debug.WriteLine("SELECT [indexEvent]      ,[dateEvent]      ,[HeureEvent]      ,[DoorNumber]      ,[userNumber]      ,[codeEvent]      ,[Flux]   FROM [controleaccessdb].[controleaccessdb].[event] e where e.doornumber in(select iddoor from [controleaccessdb].[controleaccessdb].door where idparking in (select idparking from parkingdb.dbo.parking where idparking=@idSociete)) and (dateEvent between @dateStart and @dateEnd) and codeevent " + EventCodes.GetAlertSqlString());
                using (SqlCommand cmd = new(query))
                {
                    cmd.Connection = con;
                    cmd.Parameters.Add("@idSociete", SqlDbType.Int).Value = idSociete;
